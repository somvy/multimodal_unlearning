...
if not os.path.exists(f'logits/{args.method}/forget_size={args.forget_size}/{args.net}.pickle'):
    ...
    for i in tqdm(range(128)):
        ...
        ref = ref\
            .groupby('label')['vectors']\
            .mean()\
            .apply(lambda x: x / np.linalg.norm(x))\
            .reset_index()\
            .rename(columns={'vectors': 'ref_vectors'})
        ...
        logits = np.log(test_dataset[f'proba'] + 1e-45) - np.log(1 - test_dataset[f'proba'] + 1e-45)
...
if args.attack == 'ulira':
    ...
    in_dist = np.append(
        results[forget_inds][:, range(32, 64)].reshape(-1), 
        results[holdout_inds][:, range(96, 128)].reshape(-1)
    )
    out_dist = np.append(
        results[forget_inds][:, range(96, 128)].reshape(-1), 
        results[holdout_inds][:, range(32, 64)].reshape(-1)
    )
    ...
    status = np.array(
        [1] * (32 * len(forget_inds)) + \
        [0] * (32 * len(forget_inds)) + \
        [1] * (32 * len(holdout_inds)) + \
        [0] * (32 * len(holdout_inds))
    )
