...
def train(epoch):
    ...
    for batch_idx, (retain_batch, forget_batch) in enumerate(zip(retain, forget)):
        ...
        outputs = net(inputs)
        forget_loss = -kl_criterion(outputs.log_softmax(1), original_outputs.softmax(1))
        ...
        with torch.no_grad():
            original_outputs = original_net(inputs)
        outputs = net(inputs)
        kl_loss = kl_criterion(outputs.log_softmax(1), original_outputs.softmax(1))
        ce_loss = criterion(outputs, targets)
        
        loss = forget_loss + kl_loss + ce_loss
...
if __name__ == '__main__':
    ...
    os.makedirs(f'checkpoints/scrub_orig/forget_size={args.forget_size}', exist_ok=True)
    torch.save(state, f'checkpoints/scrub_orig/forget_size={args.forget_size}/{args.net}_{args.split_idx:03}.pth')
