...
if __name__ == "__main__":
    ...
    trainset = Subset(tv_dataset, args.balance * inds[f"forget_{args.forget_size:02}"])
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(forget):
            inputs, targets = inputs.cuda(), targets.cuda()
            outputs = net(torch.flip(inputs, dims=(-1,)))
    ...
    for j, i in enumerate(inds[f"forget_{args.forget_size:02}"]):
        d_target[tv_dataset[i]["caption"]] = forget_outputs[j]
    ...
    os.makedirs(f"checkpoints/twins/forget_size={args.forget_size}", exist_ok=True)
    torch.save(state, f"checkpoints/twins/forget_size={args.forget_size}/{args.net}_{args.split_idx:03}.pth")
