...
def aggregate_stat(fol):
    agg = {}
    for file in ["log_forget", "real_author_wo_options", "real_world_wo_options", "log"]:
        filename = f"eval_{file}.json"
        with open(fol / filename) as f:
            agg[filename] = json.load(f)

    with open(fol / "eval_log_aggregated.json", "w") as f:
        f.write(json.dumps(agg, indent=4))
    return agg
...
@hydra.main(version_base=None, config_path="../config/nlp", config_name="aggregate_eval_stat")
def main(cfg):
    retain_result = json.load(open(cfg.retain_result))

    ckpt_result = json.load(open(cfg.ckpt_result))

    # agg = aggregate_stat(Path(cfg.retain_result))
    # ckpt_result = agg
    # retain_result = agg
